...
const STORAGE_KEY = "rocketgame_lb_v3";
...
const saveScore = (name, timeMs) => {
    const board = getLeaderboard();
    board.push({ name, time: timeMs, date: new Date().toLocaleDateString() });
    // Sort by fastest time (lowest first)
...
const VictoryScreen = ({ timeMs, onClose }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!name.trim()) return;
        const newBoard = saveScore(name.trim().toUpperCase(), timeMs);
        setBoard(newBoard);
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <input
                                    ...
                                    autoFocus
                                    className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-xl text-white text-center text-xl font-geist-mono uppercase tracking-wider placeholder:text-white/20 focus:outline-none focus:border-white/30 transition-colors"
                                />
                    ...
                    {phase === "leaderboard" && (
                        ...
                        >
                            ...
                            <div className="space-y-2 mb-8">
                                {board.map((entry, i) => {
                                    const isMe = entry.name === name.trim().toUpperCase() && entry.time === timeMs;
                                    return (
                                        <motion.div
                                            key={i}
